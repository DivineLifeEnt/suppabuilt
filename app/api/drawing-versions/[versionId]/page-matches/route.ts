...
export async function GET(
  ...
  const compVersionId = url.searchParams.get("comparisonVersionId");
  if (!compVersionId) {
    return Response.json({ error: { code: "BAD_REQUEST", message: "comparisonVersionId required" } }, { status: 400 });
  }
  const matches = await svc.listMatches(versionId, compVersionId);
  void user;
...
export async function POST(
  ...
  const matches = await svc.proposeMatches(versionId, compVersionId, {
    userId: user.userId,
    orgId: user.orgId,
    name: user.name,
  });
