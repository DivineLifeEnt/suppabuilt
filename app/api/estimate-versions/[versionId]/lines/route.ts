...
import { CreateEstimateLineSchema } from "@/lib/estimating/schemas";
...
export async function GET(
  ...
  let lines = await prisma.estimateLine.findMany({ where: { versionId }, orderBy: { sortOrder: "asc" } });

  if (!canViewCost) {
    lines = lines.map((l: (typeof lines)[number]) => ({
      ...l,
      unitMaterialCost: "0", laborHoursPerUnit: "0", burdenedLaborRate: "0",
...
export async function POST(
  ...
  const parsed = CreateEstimateLineSchema.safeParse(body);
  if (!parsed.success) return Response.json({ error: { code: "VALIDATION", issues: parsed.error.issues } }, { status: 400 });
  try {
    const line = await svc.createLine(versionId, parsed.data, { userId: user.userId, orgId: user.orgId, name: user.name });
