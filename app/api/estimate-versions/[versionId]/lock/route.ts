...
export async function POST(
  ...
  try { await requirePermission(user.userId, projectId, "estimate:lock"); }
  catch { return Response.json({ error: { code: "FORBIDDEN" } }, { status: 403 }); }
  try {
    const version = await reviewSvc.lock(versionId, { userId: user.userId, orgId: user.orgId, name: user.name });
